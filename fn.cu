...
__global__ void FullyConLayerForward_kernel(float input[28][28], float* Nd, float output[6][24][24], float* B, int M_height_in, int M_width_N_height_in, int N_width_in , int height_out, int width_out) {
    ...
    for(int m = 0 ; m < ceilf((float)M_width_N_height_in / TILE_WIDTH) ; ++m)
    {
        if(row < M_height_in && (m*TILE_WIDTH + tx) < M_width_N_height_in) // X
            ...
            Mds[ty][tx] = input[row][m*TILE_WIDTH + tx];
        else
            Mds[ty][tx] = 0;
